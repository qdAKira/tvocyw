			 ...
			 :confirm-loading="confirmLoading" @ok="handleOk" @cancel="handleCancel">
				<a-form :form="form" layout="inline" labelAlign="right" :labelCol="{span: 8, offset: 0}" :wrapperCol="{span: 16, offset: 0}">
					<a-row :gutter="24">
						...
						<a-col :span="12">
							<a-form-item label="污染物因子">
								<a-select placeholder="污染物因子" label-in-value v-decorator="['factor',{ rules: [{ required: true, message: '污染物因子' }] }]" >
				...
				this.form.validateFields((err, values) => {
					if (!err) {
						...
						let data = {
							...
							pollutantFactorID: values.factor.key,
							pollutantFactor: values.factor.label,
							remarks: values.remarks
						}
						addRemoteSamples(data).then(res => {
							if (res.status === '0') {
