<template>
	<sub-menu @getHeight="tableHeight($event)">
		...
		<template slot="fixed">
			<a-modal class="ant-advanced-search-form" :maskClosable="false" forceRender title="新增" :visible="modalShow" :width="600" centered
			 :confirm-loading="confirmLoading" @ok="handleOk" @cancel="handleCancel">
				<a-form :form="form" layout="inline" labelAlign="right" :labelCol="{span: 8, offset: 0}" :wrapperCol="{span: 16, offset: 0}">
					<a-row :gutter="24">
						...
						<a-col :span="12">
							<a-form-item label="降雨类别">
								<a-select placeholder="降雨类别" label-in-value v-decorator="['rainType',{ rules: [{ required: true, message: '请选择降雨类别' }] }]" >
									<a-select-option v-for="item in rainTypesList" :value="item.id" :key="item.id">{{item.name}}</a-select-option>
								</a-select>
							</a-form-item>
						</a-col>
						<a-col :span="12">
...
<script>
	...
	export default ({
		...
		created() {
			...
			getRainTypes().then(res => {
				if (res.status === '0') {
					this.rainTypesList = res.result
				} else {
					console.log(res.message)
				}
			})
		...
		methods: {
			...
			getData(data) {
				this.loading = true
				rainOrderControls(data).then(res => {
					if (res.status === '0') {
				...
				this.form.validateFields((err, values) => {
					if (!err) {
						...
						let data = {
							...
							rainTypeID: values.rainType .key,
							rainType: values.rainType .label,
							remarks: values.remarks
						}
						addRainOrderControls(data).then(res => {
							if (res.status === '0') {
