<template>
	<sub-menu @getHeight="tableHeight($event)">
		...
		<template slot="fixed">
			<modal ref="modal" title="数据统计与报表/企业数据报表/分钟数据报表" avg cou :getData="chartData"></modal>
		</template>
	</sub-menu>
</template>
	...
	export default ({
		...
		methods: {
			...
			download() {
				...
				exportCompanyMinuteData(params).then(res => {
