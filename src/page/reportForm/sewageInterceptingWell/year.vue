...
<!-- 			<a-select v-model="WellMonitorPointID" placeholder="截污井监控点名称" option-filter-prop="children"
				style="width: 120px">
				<a-select-option v-for="item in wellMonitorPointList" :value="item.id" :key="item.id">
					{{item.monitorPointName}}</a-select-option>
			</a-select> -->
		...
		<template slot="fixed">
			<modal ref="modal" title="数据统计与报表/水质数据报表/年数据报表" :wellList="wellList" avg cou :getData="chartData"></modal>
		</template>
	</sub-menu>
</template>
	...
	export default ({
		...
		methods: {
			...
			download() {
				...
				exportWellYearData(params).then(res => {
