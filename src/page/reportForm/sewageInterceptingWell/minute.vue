...
<!-- 			<a-select v-model="WellMonitorPointID" placeholder="截污井监控点名称" option-filter-prop="children" style="width: 120px">
		...
		<template slot="fixed">
			<modal ref="modal" title="数据统计与报表/水质数据报表/分钟数据报表" :wellList="wellList" avg cou :getData="chartData"></modal>
		</template>
	</sub-menu>
</template>
	...
	export default ({
		...
		methods: {
			...
			download() {
				...
				exportWellMinuteData(params).then(res => {
