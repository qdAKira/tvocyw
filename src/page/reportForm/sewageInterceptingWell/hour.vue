<template>
	<sub-menu @getHeight="tableHeight($event)">
		...
		<template slot="fixed">
			<modal ref="modal" title="数据统计与报表/水质数据报表/小时数据报表" :wellList="wellList" avg cou :getData="chartData"></modal>
		</template>
	</sub-menu>
</template>
	...
	export default ({
		...
		methods: {
			...
			download() {
				...
				exportWellHourData(params).then(res => {
