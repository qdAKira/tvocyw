...
<script>
	...
	import {
		...
		getHoleNames,
		cityElectricStatus,
		cityElectricStatusRecordsTables
	} from '@/request/api'
	...
	export default ({
			...
			return {
				...
				columns: [{
					...
					},
					{
						...
						title: '时间',
						dataIndex: 'cityElectricTimeText',
						align: 'center',
					}, {
						title: '市电状态',
						dataIndex: 'cityElectricStatus',
						align: 'center',
					}, {
						title: '备注',
						dataIndex: 'remarks',
		...
		methods: {
			...
			getData(data) {
				this.loading = true
				cityElectricStatus(data).then(res => {
					if (res.status === '0') {
			...
			download() {
				...
				cityElectricStatusRecordsTables(data).then(res => {
