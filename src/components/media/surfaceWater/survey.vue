<template>
	<div class="media-warp" :style="{left: isMove ? '-505px' : ''}">
		<div class="media-warp-title" @click="myMoveIn">地表水站点概况</div>
				...
				parkPoint().then(res => {
					if (res.status === '0') {
						...
						Object.keys(obj).forEach((key, index) => {
							series.push({
								name: key == 'fuBiaoPoint' ? '浮标站' : key == 'microPoint' ?
									'微型站' : key == 'parkIncome' ? '园区进口' : '园区总排',
								label: {
									show: true,
									formatter: '{c}',
									color: '#fff',
...
<style scoped lang="less">
	.media-warp {
		...
		width: 505px;
		height: 285px;
		background: url(../../../../static/image/yzgk.png) no-repeat left bottom / 100% 265px;
		transition: left 1s;
